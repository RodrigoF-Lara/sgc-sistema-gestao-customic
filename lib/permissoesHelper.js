...
function responderNegado(res, status, mensagem) {
...
export function exigirAdmin(req, res) {
  ...
  if (!isAdmin(nivel)) {
    responderNegado(res, 403, "Acesso negado. Apenas administradores (ADM).");
    return false;
  }
  return true;
}
...
export async function exigirPermissao(req, res, linkId, mensagem) {
  ...
  if (nivel === null) {
    responderNegado(
      res,
      401,
      "Sessão inválida. Informe o cargo do usuário (x-user-level)."
    );
    return false;
  }
  const ok = await podeAcessar(linkId, nivel);
  if (!ok) {
    responderNegado(res, 403, mensagem || `Sem permissão para: ${linkId}`);
    return false;
  }
  return true;
}

/** Passa se o usuário tiver QUALQUER um dos link_ids. */
export async function exigirQualquerPermissao(req, res, linkIds, mensagem) {
  const nivel = nivelDoRequest(req);
  if (nivel === null) {
    responderNegado(
      res,
  ...
  for (const id of ids) {
    if (await podeAcessar(id, nivel)) return true;
  }
  responderNegado(
    res,
    403,
    mensagem || `Sem permissão para: ${ids.join(" ou ")}`
  );
  return false;
}
