...
async function seedIfEmpty(pool) {
  const count = await pool.request().query(`SELECT COUNT(*) AS N FROM dbo.TB_MAPA_ESTANTE`);
  if (count.recordset[0].N > 0) return { seeded: false };
  ...
  for (const e of estantes) {
    ...
    for (const p of posicoes[e.codigo] || []) {
      await pool
        .request()
        .input("estanteId", sql.Int, id)
        .input("endereco", sql.NVarChar(40), p.endereco)
        .input("linha", sql.Int, p.linha)
        .input("col", sql.Int, p.colInicio)
        .input("span", sql.Int, p.colspan)
        .query(`
...
async function saldosPorEndereco(pool) {
  const result = await pool.request().query(`
    SELECT
      ...
      k.CODIGO,
      ISNULL(cp.DESCRICAO, '') AS DESCRICAO,
      k.ARMAZEM,
      SUM(CAST(k.SALDO AS FLOAT)) AS SALDO
    FROM dbo.KARDEX_2026_EMBALAGEM k
    LEFT JOIN dbo.CAD_PROD cp ON cp.CODIGO = k.CODIGO
    WHERE ISNULL(k.D_E_L_E_T_, '') <> '*'
      AND k.KARDEX = 2026
      AND k.SALDO > 0
      AND k.ENDERECO IS NOT NULL
      AND LTRIM(RTRIM(k.ENDERECO)) <> ''
    GROUP BY LTRIM(RTRIM(k.ENDERECO)), k.CODIGO, ISNULL(cp.DESCRICAO, ''), k.ARMAZEM
    ORDER BY LTRIM(RTRIM(k.ENDERECO)), k.CODIGO
  `);
...
async function getMapa(req, res, pool) {
  ...
  const [estantes, posicoes, saldos] = await Promise.all([
    pool.request().query(`
  ...
  const estanteList = estantes.recordset.map((e) => ({
    id: e.ID,
    codigo: e.CODIGO,
    nome: e.NOME,
    tipo: e.TIPO,
