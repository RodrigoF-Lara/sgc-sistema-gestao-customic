...
export function normalizeEndereco(v) {
  ...
  if (comPrefixo) {
    return `${comPrefixo[1]}-${Number(comPrefixo[2])}-${Number(comPrefixo[3])}`;
  }
  const soNumeros = s.match(/^(\d+)-(\d+)-(\d+)$/);
  if (soNumeros) {
    return `${Number(soNumeros[1])}-${Number(soNumeros[2])}-${Number(soNumeros[3])}`;
  }
...
async function saldosPorEndereco(pool) {
  const result = await pool.request().query(`
    SELECT
      CONVERT(varchar(40), LTRIM(RTRIM(CONVERT(varchar(40), k.ENDERECO)))) AS ENDERECO,
      CONVERT(varchar(20), k.CODIGO) AS CODIGO,
    ...
    GROUP BY CONVERT(varchar(40), LTRIM(RTRIM(CONVERT(varchar(40), k.ENDERECO)))), CONVERT(varchar(20), k.CODIGO)
  `);
...
function indexSaldos(rows) {
  ...
  for (const r of rows) {
    ...
    byNorm.get(key).push({
      endereco: enderecoAsText(r.ENDERECO),
      codigo: r.CODIGO,
