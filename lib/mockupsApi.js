...
async function criarCsv(req, res, pool) {
  ...
  try {
    ...
    if (inseridos === 0) {
      ...
      const partes = [];
      if (ignorados) partes.push(`${ignorados} já no controle`);
      if (naoCadastrados.length) partes.push(`${naoCadastrados.length} não cadastrado(s): ${naoCadastrados.slice(0, 8).join(", ")}`);
      return res.status(400).json({
        success: false,
        error: `Nenhum SKU novo. ${partes.join("; ") || "Verifique o CSV."}`,
        duplicados,
      });
    }
    ...
    if (ignorados) msg += ` ${ignorados} já existiam.`;
    if (naoCadastrados.length) {
      msg += ` ${naoCadastrados.length} não cadastrado(s) no produto: ${naoCadastrados.slice(0, 8).join(", ")}.`;
    }
    if (duplicados.length) {
      const lista = duplicados.slice(0, 12).join(", ");
      const extra = duplicados.length > 12 ? "…" : "";
      msg += ` Atenção: ${duplicados.length} código(s) repetido(s) no CSV (mantida 1 ocorrência): ${lista}${extra}.`;
    }
