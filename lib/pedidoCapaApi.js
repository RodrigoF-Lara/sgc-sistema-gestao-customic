...
import { resolverDePara, ensureDeParaTables } from "./capaDeParaApi.js";
...
async function ensureTables(pool) {
  ...
  try { await ensureDeParaTables(pool); } catch (_) { /* de/para opcional na 1a carga */ }
...
function mapPedido(row) {
  return {
    ...
    codPerso: row.COD_PERSO,
    codOrigem: row.COD_ORIGEM,
...
async function resolverCodigosPedido(pool, body) {
  let perso = String(body.codPerso || body.sku || "").trim();
  let origem = String(body.codOrigem || "").trim();
  let linha = String(body.linhaProduto || body.linha || "").trim();
  if (perso && !origem) {
    try {
      const dp = await resolverDePara(pool, perso);
      if (dp) {
        origem = dp.codOrigem;
        if (!linha) linha = dp.linha || "";
      }
    } catch (_) {
      /* de/para opcional */
    }
...
async function criar(req, res) {
  ...
  const cods = await resolverCodigosPedido(pool, req.body || {});
  ...
  const result = await pool
    ...
    .input("codPerso", sql.NVarChar(20), cods.perso)
    .input("codOrigem", sql.NVarChar(20), cods.origem)
    .input("linhaProd", sql.NVarChar(40), cods.linha)
    .query(`
      INSERT INTO dbo.COM_PEDIDO_CAPA
        (NUMERO_PEDIDO, CLIENTE, CONTATO, TELEFONE, MODELO_ID, MODELO_NOME, QUANTIDADE,
         ...
         COD_PERSO, COD_ORIGEM, LINHA_PRODUTO)
      OUTPUT INSERTED.ID
      VALUES
...
async function atualizar(req, res) {
  ...
  const st = atual.recordset[0].STATUS;
  if (st !== STATUS.RASCUNHO && st !== STATUS.EM_PRODUCAO) {
    return res.status(400).json({
      success: false,
      error: "Só é possível editar rascunho ou pedido em produção.",
    });
  }
  ...
  await pool
    ...
    .query(`
      UPDATE dbo.COM_PEDIDO_CAPA SET
        ...
        COD_PERSO=@codPerso, COD_ORIGEM=@codOrigem, LINHA_PRODUTO=@linhaProd
      WHERE ID=@id
    `);

  await salvarAnexos(pool, id, usuario, req.body.preview, req.body.arte);
  await logar(pool, id, "ATUALIZADO", usuario, st, st, "Pedido atualizado");

  const enviarAgora = !!req.body.enviarProducao && st === STATUS.RASCUNHO;
  if (enviarAgora) {
...
async function listar(req, res) {
  ...
  if (q) {
    where.push(`(
      ...
      OR p.VENDEDOR LIKE @q OR p.COD_PERSO LIKE @q OR p.COD_ORIGEM LIKE @q
      OR ('CAPA-' + RIGHT('00000'+CAST(p.ID AS VARCHAR),5)) LIKE @q
    )`);
...
async function getAnexo(req, res) {
  const id = parseId(req.query.id);
  const bruto = String(req.query.tipoAnexo || req.query.anexo || req.query.tipo || "PREVIEW").toUpperCase();
  const tipo = ["PREVIEW", "ARTE"].includes(bruto) ? bruto : "PREVIEW";
...
async function resolverModelo(req, res) {
  ...
  if (sku) {
    try {
      const pool = await getConnection();
      depara = await resolverDePara(pool, sku);
    } catch (_) {
      depara = null;
    }
  }
  ...
  return res.json({
    success: true,
    data: {
      modeloId: found && found.id,
      modeloNome: found && found.name,
      codPerso: depara ? depara.codPerso : sku || null,
      codOrigem: depara ? depara.codOrigem : null,
      linha: depara ? depara.linha : null,
      descricaoPerso: depara ? depara.descricao : null,
    },
  });
}
...
async function widgetCriar(req, res) {
  ...
  const pagina = String(req.body.origem || "").trim().slice(0, 400);
  const hoje = new Date();
  const nec = new Date(hoje.getTime() + 7 * 86400000);
  const iso = (d) => d.toISOString().slice(0, 10);
  const cods = await resolverCodigosPedido(pool, { sku, codPerso: sku, codOrigem: req.body.codOrigem });
  const obs = [
    sku ? `SKU loja (perso): ${sku}` : null,
    cods.origem ? `Código origem (produzir): ${cods.origem}` : null,
